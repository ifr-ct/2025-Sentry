...
void IFR_FDCAN_ClassDef::FDCAN_Init(FDCAN_HandleTypeDef *hfdcan)
{
	...
	if (FDCAN_Pointer[IFR_FDCAN_ID_Get(hfdcan)] != NULL && FDCAN_Pointer[IFR_FDCAN_ID_Get(hfdcan)] == this)
		return;

	FDCAN_Pointer[IFR_FDCAN_ID_Get(hfdcan)] = this;
  ...
  HAL_FDCAN_ConfigFilter(hfdcan, &fdcan_filter);			
	...
	//将IFR_FDCAN_Transmit_Callback 注册为发送fifo中断
	HAL_FDCAN_RegisterTxEventFifoCallback(hfdcan, IFR_FDCAN_Transmit_Callback);		
...
uint8_t IFR_FDCAN_ClassDef::FDCAN_Transmit(FDCAN_TxHeaderTypeDef *pHeader, uint8_t *pData)
{
	...
	else
	{
		...
		for(int i = 0; i < pHeader->DataLength; i++) TxData[Transmit_Flag % 8][i] = pData[i];
		Transmit_Flag++;
		if(Transmit_Flag - Transmit_Finish_Flag > 8)
		{
			Error_Handler();	//说明库中的缓存区满
		}
...
//中断回调函数，禁止改写和调用!!!
void IFR_FDCAN_Transmit_Callback(FDCAN_HandleTypeDef *_hfdcan, uint32_t TxEventFifoITs)
{
