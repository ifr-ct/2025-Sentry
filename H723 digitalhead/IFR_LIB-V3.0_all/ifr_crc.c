...
* @概述  				: CRC8 解析函数
*******************************************************************************/
uint8_t IFR_Get_CRC8_Check(unsigned char *pchMessage, unsigned int dwLength, unsigned char ucCRC8) // CRC8解析
{
    if (pchMessage == NULL) 
    {
      return 0xFF;  // 错误码，表示输入为空
    }
    ...
    return ucCRC8;  // 返回最终计算的 CRC8 值
}

/*******************************************************************************
* @功能     		: 生成一个这段数据 直接查表的 CRC8校验值
* @参数1        : 数据指针
* @参数2        : 数据长度（字节数）
* @返回值 			: 最终计算出来的 CRC8值
* @概述  				: 简单生成一个CRC值的函数
*******************************************************************************/
uint8_t IFR_Create_CRC8_Table(unsigned char *ptr, unsigned char len) //加密
{
    uint8_t  ucCRC = 0xff;
    if (ptr == NULL) 
    {
    ...
    while (len--)
    {
        ucCRC = CRC8_TAB[ucCRC ^ (*ptr++)];
    }
    return (ucCRC);
}

/*******************************************************************************
* @功能     		: 生成一个这段数据CRC8校验值，并且直接添加在数据末尾的最后一位
* @参数1        : 数据指针
* @参数2        : 数据长度（字节数）
* @返回值 			: 最终计算出来的 CRC8值
* @概述  				: CRC8初始值默认0xff	会把CRC8校验值添加在数组的下一位（谨慎使用，如果数组空间不够可能会加到其他地方）
*******************************************************************************/
void IFR_Append_CRC8_Check(unsigned char *pchMessage, unsigned int dwLength) 
{ 
	uint8_t ucCRC = 0xff; 
	if ((pchMessage == NULL) || (dwLength <= 2)) 	return; 
	ucCRC = IFR_Get_CRC8_Check ( (unsigned char *)pchMessage, dwLength - 1, CRC8_INIT); 
	pchMessage[dwLength - 1] = ucCRC; 
}



//CRC16 示例：x16+x12+x5+1
/*******************************************************************************
* @功能     		: 获取这段数据的 CRC16校验值
* @参数1        : 数据指针
* @参数2        : 数据长度（字节数）
* @参数3        : CRC16 初始值（一般0xff）
* @返回值 			: 最终计算出来的 CRC16值
* @概述  				: CRC16 解析函数
*******************************************************************************/
uint16_t IFR_Get_CRC16_Check(uint8_t *pchMessage, uint32_t dwLength, uint16_t uCRC) //解析
{
	uint8_t chData; 
	if (pchMessage == NULL) 
	{ 
	...
	{ 
		chData = *pchMessage++;
		(uCRC) = ((uint16_t)(uCRC) >> 8) ^ CRC16_TAB[((uint16_t)(uCRC) ^ (uint16_t)(chData)) & 0x00ff]; 
	} 
	return uCRC; 
}
...
* @概述  				: 简单生成一个CRC值的函数
*******************************************************************************/
uint16_t IFR_Create_CRC16_Table(unsigned char *ptr, unsigned char len) //加密
{
    uint16_t ucCRC = 0xffff;
    if (ptr == NULL) 
    {
      return 0xFFFF;  // 错误码，表示输入为空
    }
		
    while (len--)
    {
       ucCRC = CRC16_TAB[ucCRC ^ (*ptr++)];
    }
    return (ucCRC);
}

/*******************************************************************************
* @功能     		: 生成一个这段数据 CRC16校验值，并且直接添加在数据末尾的最后一位
* @参数1        : 数据指针
* @参数2        : 数据长度
* @返回值 			: 最终计算出来的 CRC16值
* @概述  				: CRC16初始值默认0xff	会把CRC16校验值添加在数组的下一位（谨慎使用，如果数组空间不够可能会加到其他地方）
*******************************************************************************/
void IFR_Append_CRC16_Check(uint8_t * pchMessage,uint32_t dwLength) 
{ 
	uint16_t uCRC = 0; 
	if ((pchMessage == NULL) || (dwLength <= 2)) 	return;
	uCRC = IFR_Get_CRC16_Check ( (uint8_t *)pchMessage, dwLength - 2, CRC16_INIT); 
	pchMessage[dwLength - 2] = (uint8_t)(uCRC & 0x00ff); 
	pchMessage[dwLength - 1] = (uint8_t)((uCRC >> 8)& 0x00ff);
}
