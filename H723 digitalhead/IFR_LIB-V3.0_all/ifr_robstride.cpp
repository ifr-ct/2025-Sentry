...
void IFR_LZ_Motor::LZ_Motor_Analysis(uint8_t *LZ_Data, uint32_t ID_ExtId)
{
	if ((uint8_t)((ID_ExtId & 0xFF00) >> 8) == CAN_ID)
	{
		if(CAN_ID == 1)
		{
			memcpy(big_yaw.Yaw_date, LZ_Data, 8);
		}
...
//	}
	...
	if(LZ_Pos_Info.Pattern == 2 && LZ_Pos_Info.Error_Code == 0)
	FDCAN_Pointer[Can_Motor] -> FDCAN_Transmit(&TxMessage, txdata);
}
 ...
 * @概述  			: 数据转换后高字节在前，低字节在后
 *******************************************************************************/
void IFR_LZ_Motor::LZ_Motor_Torque_control(float Torque)
{		
	Motor_Set.set_Torque = Torque;
	output = Motor_Set.set_Torque;
	
	LZ_Motor_move_control(output, 0, 0, 0, 0);
}
...
float IFR_LZ_Motor::Get_LZ_Motor_Output()
{
...
void IFR_Speed_LZ_Motor::Motor_Speed_Set(float Speed_Tar)
{
	...
	LZ_Motor_Torque_control(output);
}
 ...
 * @概述  			: None
 *******************************************************************************/
void IFR_Pos_LZ_Motor::Motor_Speed_Set(float Speed_Tar)
{
	Motor_Set.set_speed = Speed_Tar;
	output = Speed_PID.Positional_PID(Speed_Tar, LZ_Pos_Info.Speed);
	LZ_Motor_Torque_control(output);
...
void IFR_GyroControl_LZ_Motor::Motor_Angle_Set(float Angle)
{
	...
	output = Motor_Set.set_current;	
	LZ_Motor_Torque_control(Motor_Set.set_current);	
}
/*******************************************************************************
 * @功能     		: 灵足电机自行控制实例化的构造函数
 * @参数        : CAN ID
